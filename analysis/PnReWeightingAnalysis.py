...
class PnReWeightingAnalysis:
    ...
    def initialize(self): 
        ...
        self.variables = [
            'events', 
            'pn_mult',
            'nucleon_ke', 'nucleon_theta', 'nucleon_w', 'nucleon_weights', 
    ...
    def process(self, event):
        ...
        for weight in weights:
            event_weight = float(weight.getWeight())
        
        if event_weight < 1.0: print 'PN weight: %s' % float(event_weight)
        ...
        for idaughter in xrange(pn_gamma.getDaughterCount()): 
            ...
            if ((pdg_id == 2212) or (pdg_id == 2112)): 
                ...
                self.ntuple['nucleon_weights'].append(event_weight)              
    ...
    def finalize(self):
        ...
        plt.create_root_hist('nucleon_ke_w', 
                             self.ntuple['nucleon_ke'],
                             500, 0, 5000,
                             'Kinetic Energy, Inclusive, Weighted (mev)', 
                             color=self.colors[0],
                             weights=self.ntuple['nucleon_weights'])
        ...
        while theta_cut != 180:
            ...
            plt.create_root_hist('nucleon_ke_theta_cut_%s_w' % theta_cut,  
                                 self.ntuple['nucleon_ke'][self.ntuple['nucleon_theta'] > theta_cut], 
                                 500, 0, 5000,
                                 'Kinetic Energy, Inclusive, Weighted, #theta > %s (MeV)' % theta_cut, 
                                 color=self.colors[0], 
                                 weights=self.ntuple['nucleon_weights'][self.ntuple['nucleon_theta'] > theta_cut])
        ...
        plt.create_root_hist('nucleon_w_w', 
                             self.ntuple['nucleon_w'], 
                             500, 0, 5000,
        ...
        while theta_cut != 180:
            ...
            plt.create_root_hist('nucleon_w_theta_cut_%s_w' % theta_cut,  
                                 self.ntuple['nucleon_w'][self.ntuple['nucleon_theta'] > theta_cut], 
                                 500, 0, 5000,
